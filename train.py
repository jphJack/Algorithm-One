...
class Trainer:
    ...
    def _set_lr(self, epoch):
        if config.WARMUP_EPOCHS > 0 and epoch < config.WARMUP_EPOCHS:
            lr = config.LEARNING_RATE * float(epoch + 1) / float(config.WARMUP_EPOCHS)
        else:
            denom = max(1, config.NUM_EPOCHS - config.WARMUP_EPOCHS)
            progress = float(epoch - config.WARMUP_EPOCHS) / float(denom)
            cosine = 0.5 * (1.0 + math.cos(math.pi * progress))
            lr = config.MIN_LR + (config.LEARNING_RATE - config.MIN_LR) * cosine
    ...
    def _compute_ce_loss(self, logits, labels):
        ...
        loss = (1.0 - config.LABEL_SMOOTHING) * nll_loss + config.LABEL_SMOOTHING * smooth_loss
        return loss.mean()
    ...
    def train_epoch(self, epoch):
        ...
        for print_img, vein_img, labels in pbar:
            ...
            outputs = self.model(print_img, vein_img, labels=labels)
            ce_loss = self._compute_ce_loss(outputs, labels)
